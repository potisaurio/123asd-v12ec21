...
// --- SISTEMA DE COLORES (USANDO COLORES SEGUROS PARA COMPATIBILIDAD) ---
const THEMES = {
  amber: { 
    name: 'Ámbar', hex: '#f59e0b', 
    appBg: 'bg-amber-900', panelBg: 'bg-black/30', inputBg: 'bg-black/50', 
    border: 'border-amber-500/30', textMain: 'text-amber-400', textMuted: 'text-amber-200/70', 
    textValue: 'text-white', accent: 'accent-amber-500', 
    tabActive: 'bg-amber-600 text-white shadow-lg shadow-black/50', 
    tabInactive: 'bg-black/40 text-amber-100 hover:bg-black/60'
  },
  rose: { 
    name: 'Carmesí', hex: '#e11d48', 
    appBg: 'bg-rose-900', panelBg: 'bg-black/30', inputBg: 'bg-black/50', 
    border: 'border-rose-500/30', textMain: 'text-rose-400', textMuted: 'text-rose-200/70', 
    textValue: 'text-white', accent: 'accent-rose-500', 
    tabActive: 'bg-rose-600 text-white shadow-lg shadow-black/50', 
    tabInactive: 'bg-black/40 text-rose-100 hover:bg-black/60'
  },
  emerald: { 
    name: 'Esmeralda', hex: '#10b981', 
    appBg: 'bg-emerald-900', panelBg: 'bg-black/30', inputBg: 'bg-black/50', 
    border: 'border-emerald-500/30', textMain: 'text-emerald-400', textMuted: 'text-emerald-200/70', 
    textValue: 'text-white', accent: 'accent-emerald-500', 
    tabActive: 'bg-emerald-600 text-white shadow-lg shadow-black/50', 
    tabInactive: 'bg-black/40 text-emerald-100 hover:bg-black/60'
  },
  indigo: { 
    name: 'Índigo', hex: '#6366f1', 
    appBg: 'bg-indigo-900', panelBg: 'bg-black/30', inputBg: 'bg-black/50', 
    border: 'border-indigo-500/30', textMain: 'text-indigo-400', textMuted: 'text-indigo-200/70', 
    textValue: 'text-white', accent: 'accent-indigo-500', 
    tabActive: 'bg-indigo-600 text-white shadow-lg shadow-black/50', 
    tabInactive: 'bg-black/40 text-indigo-100 hover:bg-black/60'
  },
  purple: { 
    name: 'Violeta', hex: '#a855f7', 
    appBg: 'bg-purple-900', panelBg: 'bg-black/30', inputBg: 'bg-black/50', 
    border: 'border-purple-500/30', textMain: 'text-purple-400', textMuted: 'text-purple-200/70', 
    textValue: 'text-white', accent: 'accent-purple-500', 
    tabActive: 'bg-purple-600 text-white shadow-lg shadow-black/50', 
    tabInactive: 'bg-black/40 text-purple-100 hover:bg-black/60'
  },
  cyan: { 
    name: 'Cian', hex: '#06b6d4', 
    appBg: 'bg-cyan-900', panelBg: 'bg-black/30', inputBg: 'bg-black/50', 
    border: 'border-cyan-500/30', textMain: 'text-cyan-400', textMuted: 'text-cyan-200/70', 
    textValue: 'text-white', accent: 'accent-cyan-500', 
    tabActive: 'bg-cyan-600 text-white shadow-lg shadow-black/50', 
    tabInactive: 'bg-black/40 text-cyan-100 hover:bg-black/60'
  }
};
...
export default function App() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('dnd_party_data_auto_v3');
    if (saved) setCharacters(JSON.parse(saved));
  }, []);

  useEffect(() => {
    localStorage.setItem('dnd_party_data_auto_v3', JSON.stringify(characters));
  }, [characters]);
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4 font-sans">
        ...
        <div className="flex flex-col md:flex-row gap-6 w-full max-w-2xl">
          <button onClick={() => setView('PLAYER_SELECT')} className="flex-1 bg-slate-800 border-2 border-slate-700 hover:border-indigo-400 p-10 rounded-2xl transition-all shadow-xl group">
          ...
          <button onClick={() => setView('DM_DASHBOARD')} className="flex-1 bg-slate-800 border-2 border-slate-700 hover:border-rose-400 p-10 rounded-2xl transition-all shadow-xl group">
            <span className="text-6xl block mb-4 group-hover:scale-110 transition-transform">👁️</span>
  ...
  if (view === 'PLAYER_SELECT') {
    return (
      <div className="min-h-screen bg-slate-900 p-6 md:p-12 relative font-sans">
        <button onClick={() => setView('HOME')} className="text-slate-300 hover:text-white mb-8 font-bold flex items-center gap-2">← Volver al Menú</button>
        
        <div className="flex justify-between items-center mb-8 border-b border-slate-700 pb-4">
          <h2 className="text-3xl font-black text-white">Tus Personajes</h2>
          <button onClick={() => setShowCreator(true)} className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-2 px-4 rounded-xl transition-colors shadow-lg">+ Crear Personaje</button>
        </div>
          ...
          {characters.map((char) => {
            ...
            return (
              <button key={char.id} onClick={() => { setActiveCharId(char.id); setView('SHEET'); }} 
                className={`bg-slate-800 p-6 rounded-2xl border-2 text-left transition-all hover:-translate-y-1 shadow-lg flex flex-col group relative`}
              ...
              >
                ...
                <p className={`text-sm mt-1 z-10 font-bold text-slate-300`} style={{ color: charTheme.hex }}>{char.species} {char.class} (Nvl {char.level})</p>
                <div className="mt-auto pt-4 flex gap-2 z-10">
                  <span className="text-xs bg-slate-900 text-white px-3 py-1.5 rounded-lg font-bold border border-slate-700">CA: {char.ac}</span>
                  <span className="text-xs bg-slate-900 text-white px-3 py-1.5 rounded-lg font-bold border border-slate-700">HP: {char.hp.current}</span>
                </div>
              </button>
            )
          })}
        ...
        {showCreator && (
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-slate-800 p-8 rounded-3xl border border-slate-600 shadow-2xl w-full max-w-md">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <input type="text" value={newChar.name} onChange={e => setNewChar({...newChar, name: e.target.value})} className="w-full bg-slate-900 border border-slate-600 p-3 rounded-xl text-white font-bold focus:border-indigo-500 focus:outline-none" placeholder="Ej: Faelin" />
                </div>
                
                <div>
                  <label className="text-xs font-black text-slate-300 block mb-2 uppercase tracking-wider">Raza</label>
                  <select value={newChar.raceKey} onChange={e => setNewChar({...newChar, raceKey: e.target.value})} className="w-full bg-slate-900 border border-slate-600 p-3 rounded-xl text-white font-bold focus:border-indigo-500 focus:outline-none">
                ...
                <div>
                  ...
                  <select value={newChar.classKey} onChange={e => setNewChar({...newChar, classKey: e.target.value})} className="w-full bg-slate-900 border border-slate-600 p-3 rounded-xl text-white font-bold focus:border-indigo-500 focus:outline-none">
                ...
                <div>
                  ...
                  <div className="flex gap-3 bg-slate-900 p-3 rounded-xl border border-slate-600">
              ...
              <div className="flex gap-4 mt-8">
                <button onClick={() => setShowCreator(false)} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white font-bold py-3 rounded-xl transition-colors">Cancelar</button>
  ...
  if (view === 'DM_DASHBOARD') {
    return (
      <div className="min-h-screen bg-slate-900 p-6 md:p-8 text-white font-sans">
        <div className="flex justify-between items-center mb-6 border-b border-slate-700 pb-4">
          <h1 className="text-3xl font-black text-white">👁️ Panel del Dungeon Master</h1>
          <button onClick={() => setView('HOME')} className="bg-slate-700 hover:bg-slate-600 text-white px-5 py-2.5 rounded-xl font-bold transition-colors">Salir al Menú</button>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-5">
            {characters.map(char => {
              const charTheme = THEMES[char.theme || 'amber'];
              return (
                <div key={char.id} className={`bg-slate-800 p-5 rounded-2xl border border-slate-600 shadow-xl relative overflow-hidden`}>
                  ...
                  <div className="flex justify-between items-start mb-4 pl-3">
                    ...
                    <span className="bg-slate-900 px-3 py-1.5 rounded-lg text-sm font-black border border-slate-700 text-white">CA {char.ac}</span>
                  </div>
                  <div className="pl-3">
                    ...
                    <div className="w-full bg-slate-900 h-3 rounded-full overflow-hidden mb-5 border border-slate-700">
                      <div className="bg-emerald-500 h-full transition-all" style={{ width: `${Math.max(0, (char.hp.current / char.hp.max) * 100)}%` }}></div>
                    </div>
                    <div className="grid grid-cols-3 gap-3 text-center">
                      <div className="bg-slate-900 rounded-xl py-2 border border-slate-700"><span className="block text-[11px] text-slate-400 font-bold uppercase mb-1">P. Pasiva</span><span className="font-black text-lg text-white">{char.passivePerception}</span></div>
                      <div className="bg-slate-900 rounded-xl py-2 border border-slate-700"><span className="block text-[11px] text-slate-400 font-bold uppercase mb-1">Iniciativa</span><span className="font-black text-lg text-white">{getModFormatted(char.stats.dex)}</span></div>
                      <div className="bg-slate-900 rounded-xl py-2 border border-slate-700"><span className="block text-[11px] text-slate-400 font-bold uppercase mb-1">Movimiento</span><span className="font-black text-lg text-white">{char.speed} ft</span></div>
                    </div>
                  </div>
                </div>
              )
          ...
          <div className="bg-slate-800 p-6 rounded-3xl border border-slate-600 h-fit sticky top-6 shadow-xl">
            ...
            {initiativeTracker.sort((a, b) => b.roll - a.roll).map((entity, i) => (
              <div key={i} className="flex justify-between items-center bg-slate-900 p-4 rounded-xl mb-3 border border-slate-700">
            ...
            <div className="flex gap-2 mt-6">
              <input type="text" id="initName" placeholder="Monstruo / Jugador" className="w-full bg-slate-900 p-3.5 rounded-xl text-sm font-bold text-white border border-slate-700 focus:outline-none focus:border-indigo-500 placeholder:text-slate-400" />
              <input type="number" id="initRoll" placeholder="Tirada" className="w-24 bg-slate-900 p-3.5 rounded-xl text-sm font-bold text-white border border-slate-700 focus:outline-none focus:border-indigo-500 placeholder:text-slate-400 text-center" />
              <button onClick={() => {
            ...
            <button onClick={() => setInitiativeTracker([])} className="w-full mt-5 bg-slate-900 hover:bg-rose-900/50 border border-slate-700 text-sm text-slate-300 font-bold py-3 rounded-xl transition-colors">Limpiar Combate</button>
          </div>
        </div>
      </div>
    );
  ...
  // --- VISTA: FICHA DEL JUGADOR CON HUD TOTALMENTE TEÑIDO Y PERSONALIZADO ---
